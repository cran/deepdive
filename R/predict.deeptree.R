...
predict.deeptree <-
  function (object, newData, treeLeaves = NA, stackPred = NA, ...)
  {
    ...
    if (object[["preBuiltTree"]]) {
      ...
      treeLeaves = rpart.predict.leaves(object[["treeMod"]],
                                        newData)


    }

    newDataSplit <- lapply(treeLeavesGroup[treeLeavesGroup %in% unique(treeLeaves)],
                           function(tp) {
    ...
    predlist <- lapply(1:length(unique(treeLeaves)), function(mg) {
      if (!useStackPred[[mg]]) {
    ...
    ypred <- data.frame(rbindlist(predlist, use.names = T, fill = T
    ))
    ...
    if (object$modelType == "regress") {
      ...
      names(ypred) <- stringr::str_remove_all(names(ypred),
                                              "y..s.._")
      if (length(unique(ypred$ypred)) > sum(!names(ypred) %in%
                                            c("rowname", "ypred"))) {
        for (coli in unique(ypred$ypred)[!unique(ypred$ypred) %in%
                                         names(ypred)]) {
      ...
      for (icol in 1:length(ProbCols)) {
        ypred[, icol] <- ifelse(ypred$ProbSum == 0 & as.character(ypred[,
                                                                        icol]) == as.character(ypred$ypred), 1, as.character(ypred[,
                                                                                                                                   icol]))
      }
    }
    if(object$modelType=='regress'){
